...
# Define the remote volume path
VOLUME_PATH="/Volumes/StudioX"

# Check if the volume is mounted
if [ ! -d "$VOLUME_PATH" ]; then
  print_warning "Error: StudioX volume is not mounted at $VOLUME_PATH"

  # Ask the user if they want to continue anyway
  if ! gum confirm "The StudioX volume doesn't appear to be mounted. Continue anyway?"; then
    print_error "Aborting operation."
    exit 1
  fi
fi

# Set Finder to open new windows at the StudioX volume
defaults write com.apple.finder NewWindowTarget -string "PfVo"
defaults write com.apple.finder NewWindowTargetPath -string "file://$VOLUME_PATH/"
...
run_with_spinner "Restarting Finder to apply changes..." killall Finder
